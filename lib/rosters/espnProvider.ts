...
type RawPlayer = { id?: string; displayName: string; position?: { abbreviation?: string }; jersey?: string };
...
function toPlayer(raw: RawPlayer): RosterPlayer {
  return { name: raw.displayName, position: raw.position?.abbreviation ?? "", athleteId: raw.id ?? "", jersey: raw.jersey ?? "" };
}
